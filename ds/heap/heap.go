...
// in a non-empty Heap, the elements will be removed.
func (h *Heap) Heapify(elements []Element) {
	h.mu.Lock()
	defer h.mu.Unlock()

	// remove the elements from the Heap
	if len(h.elements) > 0 {
		h.elements = make([]Element, 0)
	}
...
func (h *Heap) String() string {
	...
	for i := range h.elements {
		...
		if i == len(h.elements)-1 {
			result += fmt.Sprintf("[%d]", element.GetKey())
			continue
		}
		result += fmt.Sprintf("[%d] -> ", element.GetKey())
	}
